...
#>
param(
    [string]$InstallDir = "build/QidiAdminStudio",
    [string]$OutputPath = "build/QidiAdminStudio_Windows_MSIX.msix",
    [ValidateSet("x64", "arm64")]
    [string]$Architecture = "x64",
    [string]$StagingDir = "",
    [switch]$StageOnly,
    [string]$IdentityName = "Qidi.QidiAdminStudio",
    [string]$Publisher = "CN=38F7EA55-C73B-4072-B3B2-C8E0EA15BB82",
    [string]$PublisherDisplayName = "Qidi Admin Studio"
)
...
if (-not (Test-Path (Join-Path $InstallDir 'qidi-admin-studio.exe'))) {
    throw "qidi-admin-studio.exe not found in '$InstallDir' - build the install tree first"
}
